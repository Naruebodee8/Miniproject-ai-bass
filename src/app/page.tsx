                    ...
                    {sessions.map((s) => (
                        <div key={s.id} className="relative group">
                            <button
                                ...
                                className={`w-full text-left p-4 pr-12 rounded-xl transition-all border ${s.id === currentId && activeTab === "chat"
                ...
                <header className="flex items-center justify-between px-8 py-0 h-16 bg-[#1D428A] border-b-4 border-[#FFC72C] z-20 shadow-xl shrink-0">
                    ...
                    <nav className="flex h-full">
                        <button
                            ...
                            className={`px-6 h-full flex items-center text-sm font-bold transition-all border-b-4 ${activeTab === "chat"
                        ...
                        <button
                            ...
                            className={`px-6 h-full flex items-center text-sm font-bold transition-all border-b-4 ${activeTab === "dashboard"
                                    ? "border-[#FFC72C] text-[#FFC72C] bg-white/5"
                                    : "border-transparent text-white/50 hover:text-white/80 hover:bg-white/5"
                                }`}
                        >
                                ...
                                {messages.map((msg, i) => (
                                    <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                                        <div className={`flex gap-3 max-w-[85%] ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xl shrink-0 shadow-md border-2 ${msg.role === "user" ? "bg-[#1D428A] border-[#FFC72C] text-white" : "bg-[#FFC72C] border-[#1D428A] text-[#1D428A]"
                                                }`}>
                                                {msg.role === "user" ? "🏀" : "🦓"}
                                            </div>
                                            <div className={`space-y-2 ${msg.role === "user" ? "items-end" : "items-start"}`}>
                                                <div className={`px-5 py-3 rounded-2xl text-[15px] shadow-sm font-medium ${msg.role === "user"
